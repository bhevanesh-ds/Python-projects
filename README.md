# Python-projects
 Data Science @ GITAM Visakhapatnam | Building things that (sometimes) work"
# My Python Journey 🐍💻

**B.Tech CSE (Data Science) @ GITAM Visakhapatnam | Class of 2028**

Hey there! 👋  
I'm a 2nd semester student diving into Python after surviving first-sem C 😅.  
This repo is my collection of beginner projects, experiments, and everything I'm learning.


## 🚀 My Projects

- 🧮 **Calculator**
  - Basic calculator using Python
  - 👉 [Open calculator Project](calculator /calculator.py)


## 🛠️ Currently Learning
- Python basics & control flow
- Functions, lists, dictionaries
- File handling & error handling
- Next: OOP, pandas, numpy (DS track 🔥)

## 🌱 Goals
- Build 10+ projects this semester
- Start competitive programming

Thanks for stopping by! ⭐ the repo if you like it 🤝

*Last updated: December 23, 2025*
